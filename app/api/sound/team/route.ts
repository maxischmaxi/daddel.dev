...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    ...
    const targets = parseFrequencyArray(o.targets, 5);
    ...
    const guesses = parseFrequencyArray(creator.guesses, 5);
    ...
    await db.batch([
      db.insert(soundTeamGames).values({
        id,
        createdAt: now,
        creatorName: name,
      ...
      db.insert(soundTeamScores).values({
        gameId: id,
        clientId,
        name,
        totalScore: total,
    ...
    console.error("POST /api/sound/team failed", e);
