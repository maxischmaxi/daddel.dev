...
export function FinalTeamParticipant({
  totalScore,
  targets,
  guesses,
  scores,
  ...
  const shareUrl = lobby ? buildShareUrl(lobby.id) : null;
      ...
      {state === "error" && (
        <div className="flex flex-col items-center gap-2 px-2 py-3">
          <p className="text-center text-xs text-red-400">
            {dict.final.team.sendFailed}: {errorMessage ?? dict.common.unknownError}
          </p>
          <button
            type="button"
